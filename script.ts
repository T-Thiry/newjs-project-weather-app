  ...
  function displayWeeklyWeather(forecastList) {
    ...
    forecastList.forEach((forecast, index) => {
      if (index < rows.length) {
        ...
        if (dayCell) dayCell.textContent = forecast.day;
        ...
        if (tempCell) tempCell.textContent = `${Math.round(forecast.temp)}°C`;

        const windCell = rows[index].querySelector(`#windday${index + 1}`);
        if (windCell) windCell.textContent = `${forecast.wind} m/s`;
      }
    });
  }
